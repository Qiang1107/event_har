...
def main(config_path, best_model_path, log_path, pretrained_path=None):
    ...
    if model_type == 'pointnet2':
        train_ds = ESequenceDataset(
            data_root          = ds['train_dir'],
            window_size_us     = ds['window_size_us'],
            stride_us          = ds['stride_us'],
            max_points         = ds['max_points'],
            time_dimension     = ds['time_dimension'],
            enable_augment     = ds['enable_augment']
        )
        val_ds = ESequenceDataset(
